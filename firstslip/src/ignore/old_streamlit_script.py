...
def streamlit_viz():
    ...
    st.image(image=str(im_path), caption="Original")

    depth_map, focal_length = get_depth(im_path=im_path)
    pointcloud_df = depth_map_to_point_cloud(depth_map=depth_map, focal_length=focal_length)
    ...
    deck = pydeck.Deck(layers=point_cloud_layer, initial_view_state=view_state, views=[view])
    deck_html_str = deck.to_html(as_string=True, css_background_color="#add8e6")
    components.html(html=deck_html_str, height=800)


def plotly_viz(pointcloud_df: pd.DataFrame) -> None:
    fig = px.scatter_3d(data_frame=pointcloud_df, x="x", y="y", z="z", title="Point Cloud Visualization")
...
def main() -> None:
    ...
    depth_map, focal_length = get_depth(im_path=im_path)
    pointcloud_df = depth_map_to_point_cloud(depth_map=depth_map, focal_length=focal_length)
    plotly_viz(pointcloud_df=pointcloud_df)
